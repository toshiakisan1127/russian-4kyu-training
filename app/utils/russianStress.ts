...
const verbStressOverrides: Record<string, readonly string[]> = {
  'знать': ['зна́ю', 'зна́ешь', 'зна́ет', 'зна́ем', 'зна́ете', 'зна́ют'],
  ...
  'висеть': ['вишу́', 'ви́сишь', 'ви́сит', 'ви́сим', 'ви́сите', 'ви́сят'],
  ...
  'становиться': ['становлю́сь', 'стано́вишься', 'стано́вится', 'стано́вимся', 'стано́витесь', 'стано́вятся'],
  'искать': ['ищу́', 'и́щешь', 'и́щет', 'и́щем', 'и́щете', 'и́щут'],
  'звать': ['зову́', 'зовёшь', 'зовёт', 'зовём', 'зовёте', 'зову́т'],
  'существовать': ['существу́ю', 'существу́ешь', 'существу́ет', 'существу́ем', 'существу́ете', 'существу́ют'],
  ...
  'рисовать': ['рису́ю', 'рису́ешь', 'рису́ет', 'рису́ем', 'рису́ете', 'рису́ют'],
...
const adjectiveStressOverrides: Record<string, string> = {
  'злая': 'зла́я',
  'злое': 'зло́е',
  'злые': 'злы́е',
  'злого': 'зло́го',
...
export const stressAdjectiveForm = (form: string, stressedLemma: string) => {
  ...
  const adjectiveLemma = stripStress(stressedLemma)
  return alternatives
    .map((alternative) => {
      const noteMatch = alternative.match(/^(.*?)(（.*）)?$/u)
      const word = noteMatch?.[1]?.trim() ?? alternative
      const note = noteMatch?.[2] ?? ''
      const plainWord = stripStress(word)
      const accentedWord = adjectiveLemma === 'злой'
        ? adjectiveStressOverrides[plainWord] ?? plainWord
        : stressLikeLemma(word, stressedLemma)
      return `${accentedWord}${note}`
    })
    .join(' / ')
}
