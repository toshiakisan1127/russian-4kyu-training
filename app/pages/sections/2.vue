...
const createQuestionSet = () => {
  const statuses = getQuestionStatuses(section2Questions.map((question) => question.id))
  const buckets: Record<QuestionStatus, typeof section2Questions> = {
    new: [],
    review: [],
    learning: [],
    mastered: [],
  ...
  const queues: Record<QuestionStatus, typeof section2Questions> = {
  ...
  const selected: typeof section2Questions = []
  ...
  return shuffle(selected).map((question) => {
    const correctSourceId = question.choices[question.answer]!.sourceId
    const choices = shuffle(question.choices)

    return {
      ...question,
      choices,
      answer: choices.findIndex((choice) => choice.sourceId === correctSourceId),
    }
  })
}
...
const stressLabel = (position: number) => `第${position}音節`
      ...
      <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-sky-100/60 sm:p-8">
        <header class="mb-7 flex items-start justify-between gap-4">
          <div>
            ...
            <h1 class="text-2xl font-black tracking-tight sm:text-3xl">第II問・アクセント</h1>
        ...
        <div v-if="!completed">
          ...
          <div class="grid gap-3 sm:grid-cols-2">
            ...
            >
              ...
              <span class="min-w-0 flex-1">
                <span class="block text-2xl font-bold" style="font-family: 'PT Serif', Georgia, serif">
                  {{ choice.word }}
                </span>
              </span>
              <span v-if="answered && index === currentQuestion.answer" class="shrink-0 rounded-full bg-sky-600 px-2 py-1 text-[11px] font-black text-white">
                ✓ 正解
              </span>
              <span v-else-if="answered && selectedAnswer === index" class="shrink-0 rounded-full border-2 border-dashed border-amber-600 bg-amber-100 px-2 py-1 text-[11px] font-black text-amber-950">
                × 回答
              </span>
            </button>
          </div>
          ...
          <section v-if="answered" class="mt-7 border-t border-slate-200 pt-6" aria-live="polite">
            ...
            <div class="mb-6 rounded-2xl border border-sky-100 bg-sky-50 px-4 py-3.5">
              ...
              <p class="m-0 font-bold leading-7 text-slate-800">
                他の3語は {{ stressLabel(currentQuestion.commonStressPosition) }}、正解だけ {{ stressLabel(currentQuestion.oddStressPosition) }} にアクセントがあります。
              </p>
            </div>

            <div class="mb-7">
              <h2 class="mb-2 text-base font-black">4語のアクセントを確認</h2>
              <article
                v-for="(choice, index) in currentQuestion.choices"
                :key="`detail-${choice.sourceId}`"
                class="border-t border-slate-200 py-4"
              >
                <div class="flex items-center justify-between gap-3">
                  <div class="min-w-0">
                    <div class="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                      <strong class="text-xl" style="font-family: 'PT Serif', Georgia, serif">
                        {{ index + 1 }}. {{ choice.stressedWord }}
                      </strong>
                      <span class="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-black text-slate-700">
                        {{ stressLabel(choice.stressPosition) }}
                      </span>
                    </div>
                    <p class="mt-1 mb-0 text-sm font-medium text-slate-500">{{ choice.meaning }}</p>
                  </div>
                  <button
                    type="button"
        ...
        <section v-else class="py-10 text-center">
          <p class="mb-2 text-xs font-black tracking-[0.14em] text-sky-700 uppercase">Section II Result</p>
          ...
          <p class="mx-auto mb-0 max-w-md leading-7 text-slate-600">
            100問プールから、要復習・新規を優先して10問ずつアクセントを定着させる。
          </p>
