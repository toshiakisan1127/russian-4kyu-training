...
const createQuestionSet = () => {
  const statuses = getQuestionStatuses(section8Questions.map((question) => question.id))
  const buckets: Record<QuestionStatus, typeof section8Questions> = {
    new: [],
    review: [],
    learning: [],
    mastered: [],
  ...
  section8Questions.forEach((question) => {
    buckets[statuses[question.id] ?? 'new'].push(question)
  })

  const queues: Record<QuestionStatus, typeof section8Questions> = {
  ...
  const selected: typeof section8Questions = []
...
const choiceClasses = (choice: string) => {
  ...
  if (!answered.value) return `${base} border-slate-300 bg-white hover:-translate-y-0.5 hover:border-sky-400 hover:bg-sky-50`
  if (choice === currentQuestion.value.correctAnswer) return `${base} border-2 border-solid border-sky-500 bg-sky-50 shadow-[inset_5px_0_0_#0ea5e9]`
      ...
      <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-sky-100/60 sm:p-8">
        <header class="mb-7 flex items-start justify-between gap-4">
          <div>
            ...
            <h1 class="text-2xl font-black tracking-tight sm:text-3xl">第VIII問・過去形と未来形</h1>
            <p class="mt-1 mb-0 text-xs font-bold text-slate-500">120問から習熟度に合わせて10問</p>
          </div>
        ...
        <div v-if="!completed">
          <div class="mb-4 flex flex-wrap items-center gap-2">
            ...
            <span class="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-xs font-black text-slate-600">4択</span>
            <span class="rounded-full border border-sky-200 bg-sky-50 px-2.5 py-1 text-xs font-black text-sky-800">{{ tenseLabel }}</span>
          </div>

          <div class="mb-6 rounded-2xl bg-slate-100 px-4 py-3.5">
            <p class="m-0 text-sm font-bold leading-6 text-slate-700">
              現在形の文を、指定された時制に直すときの正しい語形を選びなさい。
            </p>
          </div>

          <div class="mb-4 rounded-2xl border border-slate-200 bg-white p-4">
            <p class="mb-1 text-xs font-black tracking-[0.12em] text-slate-500 uppercase">Present</p>
            <div class="flex items-start justify-between gap-4">
              <p class="m-0 text-lg font-bold leading-8" style="font-family: 'PT Serif', Georgia, serif">{{ currentQuestion.sourceSentence }}</p>
              <button type="button" class="grid size-9 shrink-0 place-items-center rounded-full border border-slate-200 bg-slate-50 disabled:opacity-40" :disabled="!speechSupported" @click="speak(currentQuestion.sourceSentence)">🔊</button>
            </div>
          </div>

          <div class="mb-6 rounded-2xl border border-sky-100 bg-sky-50 p-5">
            <div class="mb-3 flex items-center gap-3">
              <strong class="text-lg" style="font-family: 'PT Serif', Georgia, serif">{{ currentQuestion.infinitive }}</strong>
              <span class="text-sm font-bold text-slate-500">{{ currentQuestion.meaning }}</span>
            </div>
            <p class="m-0 text-2xl font-bold leading-10 sm:text-3xl" style="font-family: 'PT Serif', Georgia, serif">{{ currentQuestion.prompt }}</p>
          </div>
          ...
          <div class="grid gap-3">
            ...
            >
              ...
              <strong class="text-lg leading-7" style="font-family: 'PT Serif', Georgia, serif">{{ choice }}</strong>
              <span v-if="answered && choice === currentQuestion.correctAnswer" class="ml-auto shrink-0 rounded-full bg-sky-600 px-2 py-1 text-[11px] font-black text-white">✓ 正解</span>
          ...
          <section v-if="answered" class="mt-7 border-t border-slate-200 pt-6" aria-live="polite">
            <div class="mb-5 flex items-center gap-4 rounded-2xl border-2 p-4" :class="isCorrect ? 'border-solid border-sky-500 bg-sky-50' : 'border-dashed border-amber-500 bg-amber-50'">
              ...
              <div>
                ...
                <p class="m-0 text-sm text-slate-700">正解：<strong>{{ currentQuestion.correctAnswer }}</strong></p>
              </div>
            </div>

            <div class="mb-5 rounded-2xl border border-sky-100 bg-sky-50 p-4">
              <p class="mb-2 text-xs font-black tracking-[0.12em] text-sky-700 uppercase">Full Sentence</p>
              <div class="flex items-start justify-between gap-4">
                <p class="m-0 text-xl font-bold leading-8" style="font-family: 'PT Serif', Georgia, serif">{{ currentQuestion.fullAnswer }}</p>
                <button type="button" class="grid size-9 shrink-0 place-items-center rounded-full border border-sky-200 bg-white disabled:opacity-40" :disabled="!speechSupported" @click="speak(currentQuestion.fullAnswer)">🔊</button>
              </div>
            </div>
        ...
        <section v-else class="py-10 text-center">
          <p class="mb-2 text-xs font-black tracking-[0.14em] text-sky-700 uppercase">Section VIII Result</p>
          <h2 class="mb-3 text-4xl font-black text-sky-800 sm:text-5xl">{{ correctCount }} / {{ questionSet.length }}</h2>
          <p class="mx-auto mb-0 max-w-md leading-7 text-slate-600">過去形の性・数一致と、不完了体の「быть + 不定形」の未来をセットで定着させる。</p>
