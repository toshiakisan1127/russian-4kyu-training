...
const createQuestionSet = () => {
  const statuses = getQuestionStatuses(russianToJapaneseQuestions.map((question) => question.id))
  const buckets: Record<QuestionStatus, typeof russianToJapaneseQuestions> = {
    new: [],
    review: [],
    learning: [],
    mastered: [],
  ...
  russianToJapaneseQuestions.forEach((question) => {
    buckets[statuses[question.id] ?? 'new'].push(question)
  })

  const queues: Record<QuestionStatus, typeof russianToJapaneseQuestions> = {
  ...
  const selected: typeof russianToJapaneseQuestions = []
      ...
      <div class="rounded-3xl border border-slate-200 bg-white p-5 shadow-xl shadow-sky-100/60 sm:p-8">
        ...
        <div v-if="!completed">
          ...
          <div class="mb-6 rounded-2xl bg-slate-100 px-4 py-3.5">
            <p class="m-0 text-sm font-bold leading-6 text-slate-700">
              次のロシア語を日本語に訳しなさい。自分の訳を書いたら模範訳と比べて自己採点。
            </p>
          </div>
          ...
          <div class="mb-5 rounded-2xl border border-sky-100 bg-sky-50 p-5">
            <div class="flex items-start justify-between gap-4">
              ...
              <button
                ...
                aria-label="ロシア語文を読み上げる"
                @click="speak(currentQuestion.sentence)"
              >🔊</button>
            </div>
          </div>

          <label class="mb-2 block text-sm font-black text-slate-700" for="translation-draft">自分の訳</label>
          <textarea
            id="translation-draft"
            v-model="draft"
            rows="4"
            class="mb-4 w-full resize-y rounded-2xl border border-slate-300 bg-white px-4 py-3 leading-7 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
            placeholder="日本語訳を書いてみる（空欄でも模範訳は確認できます）"
            :disabled="graded"
          />
          ...
          <section v-else class="mt-6 border-t border-slate-200 pt-6" aria-live="polite">
            <div class="mb-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4">
              ...
              <p class="m-0 text-lg font-bold leading-8 text-slate-900">{{ currentQuestion.translation }}</p>
            </div>

            <div v-if="draft.trim()" class="mb-5 rounded-2xl border border-slate-200 bg-white p-4">
              <p class="mb-2 text-xs font-black tracking-[0.12em] text-slate-500 uppercase">Your Translation</p>
              <p class="m-0 leading-7 text-slate-800">{{ draft }}</p>
            </div>
            ...
            <div class="mb-5 grid gap-4 sm:grid-cols-2">
              ...
              <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <p class="mb-3 text-sm font-black text-slate-900">文法ポイント</p>
                <ul class="m-0 space-y-2 pl-5 text-sm leading-6 text-slate-700">
        ...
        <section v-else class="py-10 text-center">
          ...
          <p class="mx-auto mb-0 max-w-md leading-7 text-slate-600">
            「できた」にした問題は定着へ、「要復習」は次回優先して出題されます。
          </p>
          <button
            type="button"
