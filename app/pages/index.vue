      ...
      <section class="mb-12">
        <div class="mb-5 flex items-end justify-between gap-4">
          <div>
            ...
            <h2 class="m-0 text-2xl font-black">分野別トレーニング</h2>
          </div>
          <span class="text-sm font-bold text-slate-500">苦手を個別に攻略</span>
        </div>
        ...
        <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          <template v-for="item in trainingItems" :key="item.title">
            <NuxtLink
              ...
              :to="item.to"
              class="group flex min-h-44 flex-col justify-between rounded-3xl border border-indigo-200 bg-white p-5 shadow-sm shadow-indigo-100 transition hover:-translate-y-1 hover:border-indigo-400 hover:shadow-lg hover:shadow-indigo-100"
            >
              <div>
            ...
            <article
              v-else
              class="flex min-h-44 flex-col justify-between rounded-3xl border border-slate-200 bg-slate-100/70 p-5 text-slate-500"
            >
              <div>
